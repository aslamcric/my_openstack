...
const BuyPackages = (props) => {
    ...
    const customPrice = useMemo(() => {
        const hourlyBaseRate = 0.02;
        const resourceBase = (customSpecs.core * 75) + (customSpecs.ram * 25);
    ...
    // const customPrice = useMemo(() => {
    //     const basePerHour = 5;      // fixed platform cost
    //     const coreRate = 10;        // per core per hour
    //     const ramRate = 5;          // per GB per hour

    //     const hourlyPrice = basePerHour + (customSpecs.core * coreRate) + (customSpecs.ram * ramRate);

    //     return Math.round(
    //         hourlyPrice *
    //         customSpecs.time *
    //         STORAGE_CONFIG[customSpecs.storageType].multiplier
    //     );
    // }, [customSpecs]);
                        ...
                        <table className={styles.reviewTable}>
                            ...
                            <tbody>
                                {Object.values(cart).map((item, idx) => {
                                    const cartKey = `${item.name}-${item.storageType}`;
                                    return (
                                        <tr key={idx}>
                                            <td>{item.name}</td>
